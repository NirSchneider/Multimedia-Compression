...
if __name__ == '__main__':
    ...
    cv2.namedWindow('PSF DFT', 0)
    cv2.createTrackbar('angle', win, int(opts.get('--angle', 150)), 250, update)
    cv2.createTrackbar('d', win, int(opts.get('--d', 22)), 150, update)
    cv2.createTrackbar('SNR (db)', win, int(opts.get('--snr', 40)), 50, update)
    update(None)
